...
class Client:
    ...
    def connect_and_auth(cls, token: str, name: str, port: int = PORT) -> "Client":
        c = cls(port)
        c.send({"type": "HELLO", "token": token, "name": name, "ver": 4})
        c.expect("CHAR_LOAD")
        return c
...
def test_auth_new():
    token = fresh_token()
    c = Client()
    c.send({"type": "HELLO", "token": token, "name": "NewPlayer", "ver": 4})
...
def test_auth_returning():
    ...
    c2 = Client()
    c2.send({"type": "HELLO", "token": token, "name": "ReturnPlayer", "ver": 4})
...
def test_auth_bad_token():
    c = Client()
    c.send({"type": "HELLO", "token": "short", "name": "Hacker", "ver": 4})
    pkt = c.expect("KICK")
...
def test_reveal_markers_on_join():
    ...
    token = fresh_token()
    c = Client()
    c.send({"type": "HELLO", "token": token, "name": "MapPlayer", "ver": 4})
...
def test_char_save_skill_clamp():
    ...
    time.sleep(0.1)
    b = Client()
    b.send({"type": "HELLO", "token": token, "name": "ClampTest", "ver": 4})
...
def test_char_save_level_clamp():
    ...
    time.sleep(0.1)
    b = Client()
    b.send({"type": "HELLO", "token": token, "name": "LevelClamp", "ver": 4})
    pkt = b.expect("CHAR_LOAD")
    level = pkt.get("level")
...
def test_zero_stats_rejected():
    ...
    time.sleep(0.1)
    b = Client()
    b.send({"type": "HELLO", "token": token, "name": "ZeroStats", "ver": 4})
    pkt = b.expect("CHAR_LOAD")
...
def test_start_cell_until_tutorial_done():
    ...
    b = Client()
    b.send({"type": "HELLO", "token": token, "name": "SewerBound", "ver": 4})
    pkt = b.expect("CHAR_LOAD")
...
def test_duplicate_name_create():
    ...
    a = Client(PORT)
    a.send({"type": "HELLO", "token": fresh_token(), "name": "Bendu Olo", "ver": 4})
    pa = a.expect("CHAR_LOAD")

    b = Client(PORT)
    b.send({"type": "HELLO", "token": fresh_token(), "name": "Bendu Olo", "ver": 4})
...
def test_rename_to_taken_name():
    ...
    c = Client()
    c.send({"type": "HELLO", "token": token, "name": "NameOwner", "ver": 4})
    pkt = c.expect("CHAR_LOAD")
...
def test_duplicate_token_rejected():
    ...
    b = Client()
    b.send({"type": "HELLO", "token": token, "name": "TokenThief", "ver": 4})
    pkt = b.expect("KICK")
...
def test_weather_sync():
    ...
    c = Client()
    c.send({"type": "HELLO", "token": fresh_token(), "name": "WeatherC", "ver": 4})
    pkts = c.recv_all(1.0)
    types = [p.get("type") for p in pkts]
...
def test_bounty():
    ...
    time.sleep(0.1)
    b = Client()
    b.send({"type": "HELLO", "token": token, "name": "Criminal", "ver": 4})
    pkt = b.expect("CHAR_LOAD")
    ...
    time.sleep(0.1)
    c = Client()
    c.send({"type": "HELLO", "token": token, "name": "Criminal", "ver": 4})
    pkt = c.expect("CHAR_LOAD")
    bounties = pkt.get("bounties", {})
...
def test_npc_pos_relay():
    ...
    npcs[5]["ref"] = 0x14                            # PlayerRef: must be dropped
    a.send({"type": "NPC_POS", "cell": cell_key, "npcs": npcs})
    pkt = b.expect("NPC_POS")
    got = pkt.get("npcs", [])
    refs = [e["ref"] for e in got]
    assert 5003 not in refs and 0x14 not in refs, f"NaN/PlayerRef filtering wrong: {refs}"
    assert len(got) == 8, f"cap wrong: {refs}"

    # PlayerRef can never be killed-synced either
    a.send({"type": "NPC_KILLED", "ref_id": 0x14, "cell": cell_key})
    b.expect_none("NPC_KILLED", timeout=0.3)
